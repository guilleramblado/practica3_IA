...
double AIPlayer::MiValoracion(const Parchis &estado, int jugador){
    ...
    else if (ganador == oponente)
    {
        ...
        if(estado.isEatingMove()){
            if(get<0>(estado.getLastAction())==my_colors[0] or get<0>(estado.getLastAction())==my_colors[1]){
                //si soy yo el que se ha comido una ficha
                puntuacion_jugador+=60;
            }
            else{
                //el color que tenia el turno para el que se ha comido una ficha era del oponente, el oponente ha comido ficha
        ...
        //comparo mis fichas en casilla de meta con las del oponente en casilla de meta
        /*
        int fichas_jugador_meta=estado.piecesAtGoal(my_colors[0])+estado.piecesAtGoal(my_colors[1]);
        int fichas_oponente_meta=estado.piecesAtGoal(op_colors[0])+estado.piecesAtGoal(op_colors[1]);
        if(fichas_jugador_meta>fichas_oponente_meta){
            //tengo mas fichas en meta que el oponente
            puntuacion_jugador=(fichas_jugador_meta-fichas_oponente_meta)*10;
        }
        else if(fichas_oponente_meta>fichas_jugador_meta){
            puntuacion_oponente=(fichas_oponente_meta-fichas_jugador_meta)*10;
        }
        */
...
double AIPlayer::Poda_AlfaBeta(const Parchis &actual, int jugador, int profundidad, int profundidad_max, color &c_piece, int &id_piece, int &dice, double alpha, double beta, double (*heuristic)(const Parchis &, int)) const{
    ...
    //MAX sera el mismo jugador que invoco inicialmente a poda alfa-beta
    if(actual.getCurrentPlayerId()==jugador){
        ...
        while(!(siguiente_hijo==actual)){
            ...
            aux=Poda_AlfaBeta(siguiente_hijo,jugador,profundidad+1,profundidad_max,aux_c_piece,aux_id_piece,aux_dice,alpha,beta,MiValoracion);
